...
export function PromoBarCarousel({ features }: { features: any[] }) {
  ...
  if (features.length === 0) {
    return (
      <Link href="#" className="inline-flex items-center gap-2 group transition-colors">
        ...
        <span className="text-white text-xs sm:text-sm font-bold leading-none group-hover:underline decoration-white/50 underline-offset-4">
  ...
  return (
    <div className="relative h-5 flex items-center justify-center w-full max-w-4xl mx-auto overflow-visible px-4">
      {features.map((item, index) => (
        ...
        >
          ...
          <span className="text-white text-xs sm:text-sm font-bold leading-none text-center truncate max-w-[280px] sm:max-w-md md:max-w-xl group-hover:underline decoration-white/50 underline-offset-4 pointer-events-auto cursor-pointer">
            Featured: {item.title}
          </span>
